/* ============================================================
   TalentTie — Landing (marketing) page — iteration 2
    ...
    <header className="land-nav">
      <div className="container" style={{ height: 70, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button className="btn btn-quiet btn-sm land-login" onClick={() => go("login")}>Log in</button>
          <button className="btn btn-primary btn-sm" onClick={() => go("login")}>Get started</button>
        </div>
      </div>
    </header>
  );
...
function StackingVisual() {
  ...
  useEffect(() => {
    ...
    const cycle = () => {
      if (!alive) return;
      setFilled(false);
      setTimeout(() => alive && setFilled(true), 250);
    };
    cycle();
  ...
  return (
    <div className="stack-visual">
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 8, margin: "4px 0 12px", whiteSpace: "nowrap" }}>
        <span className="tnum" style={{ fontSize: 40, fontWeight: 800, color: "var(--primary)", letterSpacing: "-.03em", transition: "all .8s var(--ease)" }}>{filled ? total : 0}</span>
      ...
      <div className="bar" style={{ height: 18 }}>
        {segs.map((s, i) => (
          <div key={i} className="bar-seg" style={{
            ...
            transitionDelay: `${i * 0.22}s`, transitionDuration: ".7s",
          }} />
        ))}
      </div>
        ...
        {segs.map((s, i) => (
          <div key={i} className="svl-row" style={{ transitionDelay: `${i * 0.22 + 0.2}s`, opacity: filled ? 1 : 0.35 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 9, minWidth: 0 }}>
              ...
              <span style={{ fontWeight: 600, fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.company}</span>
            </span>
            <span className="tnum" style={{ fontWeight: 700, fontSize: 13, flex: "none" }}>{s.hours}h</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="card card-pad hover-lift" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <span className="tnum" style={{ fontSize: 30, fontWeight: 800, color: "var(--line)", letterSpacing: "-.04em" }}>0{n}</span>
      </div>
...
function LandingPage({ go }) {
  const businesses = ["Forest City Roasters", "Thames Valley Marketing Group", "Northern Currents Studio", "Riverbend Design Co.", "Innovation Works", "Powerhouse Brewing Co."];
  return (
        ...
        <div className="container" style={{ position: "relative", paddingTop: 64, paddingBottom: 56 }}>
          <div className="land-hero-grid">
            <div style={{ maxWidth: 560 }}>
              <div className="land-badge"><span className="dot" style={{ background: "var(--teal)" }} /> University partners coming soon — Fanshawe &amp; Western</div>
              ...
              <p style={{ fontSize: 18.5, color: "var(--ink-2)", marginTop: 20, lineHeight: 1.5, maxWidth: 500 }}>
                TalentTie lets London students complete their required co-op hours across several
                local businesses — we total every hour and issue one completion certificate.
              </p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 28 }}>
                <button className="btn btn-primary btn-lg" onClick={() => go("login", "student")}><Icon name="cap" size={19} /> I'm a Student</button>
                <button className="btn btn-ghost btn-lg" onClick={() => go("login", "employer")}><Icon name="building" size={18} /> I'm an Employer</button>
              </div>
              <div style={{ display: "flex", gap: 22, marginTop: 26, color: "var(--muted)", fontSize: 13.5, fontWeight: 600, flexWrap: "wrap" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 7 }}><Icon name="check" size={15} style={{ color: "var(--green)" }} /> Free for students</span>
                <span style={{ display: "flex", alignItems: "center", gap: 7 }}><Icon name="check" size={15} style={{ color: "var(--green)" }} /> Local London employers</span>
              </div>
            </div>
            <div className="land-hero-visual"><StackingVisual /></div>
          </div>
        </div>
      </section>
      ...
      <section className="land-proof">
        <div className="container">
          <div className="land-proof-grid">
            <div>
              <div className="tnum" style={{ fontSize: 30, fontWeight: 800, color: "#fff", letterSpacing: "-.02em" }}>60,000+</div>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ color: "rgba(255,255,255,.78)", fontSize: 12.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 10 }}>Hiring across London neighbourhoods</div>
              <div className="land-hoods">
                {["Downtown", "Old East Village", "Wortley Village", "SoHo", "Hyde Park", "Masonville", "Byron", "Westmount"].map(n => (
                  <span key={n} className="land-hood">{n}</span>
                ))}
              </div>
            </div>
          </div>
          <div className="land-logos">
            ...
            {businesses.map(b => (
              <span key={b} className="land-logo"><CoTile name={b} size={26} radius={7} /> <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.replace(" Group", "").replace(" Co.", "")}</span></span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="land-signature">
        <div className="container">
          <div className="land-sig-grid">
            <div>
              <div className="eyebrow" style={{ color: "var(--teal)" }}>Signature feature</div>
              <h2 style={{ fontSize: "clamp(27px,3.2vw,40px)", marginTop: 12, lineHeight: 1.08 }}>
                No single shop has to take all your hours.
              </h2>
              <p style={{ color: "var(--ink-2)", fontSize: 16.5, marginTop: 18, lineHeight: 1.55, maxWidth: 440 }}>
                A café offers 120 hours, a marketing agency 150, a studio 100. TalentTie adds them
                up, tracks approvals from each employer, and gets you across the line — then issues
                a single, verifiable certificate.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 22 }}>
                {[["Partial hours welcome — even 60 hours helps", "layers"], ["Every employer approves their own hours", "check"], ["One certificate, recognized across placements", "award"]].map(([t, ic]) => (
                  <div key={t} style={{ display: "flex", alignItems: "center", gap: 11, fontSize: 14.5, fontWeight: 600, color: "var(--ink-2)" }}>
                    <span style={{ width: 30, height: 30, borderRadius: 8, background: "#fff", color: "var(--primary)", display: "grid", placeItems: "center", flex: "none", boxShadow: "var(--sh-xs)" }}><Icon name={ic} size={16} /></span>{t}
                  </div>
                ))}
              </div>
              <button className="btn btn-primary" style={{ marginTop: 26 }} onClick={() => go("login", "student")}>See your hours dashboard <Icon name="arrowRight" size={16} /></button>
            </div>
            <div className="land-sig-visual"><StackingVisual /></div>
          </div>
        </div>
      </section>
      ...
      <section className="container" style={{ paddingTop: 64, paddingBottom: 64 }}>
        <div className="land-cta">
          <div style={{ position: "relative", zIndex: 1 }}>
            <span className="land-uni-badge"><span className="dot" style={{ background: "#5eead4" }} /> University partners coming soon — Fanshawe &amp; Western</span>
            <h2 style={{ color: "#fff", fontSize: "clamp(26px,3vw,36px)", marginTop: 18, lineHeight: 1.1, maxWidth: 520 }}>Ready to turn your hours into a certificate?</h2>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12, marginTop: 24 }}>
              <button className="btn btn-onteal btn-lg" onClick={() => go("login", "student")}><Icon name="cap" size={18} /> I'm a Student</button>
              <button className="btn btn-onteal-ghost btn-lg" onClick={() => go("login", "employer")}><Icon name="building" size={18} /> I'm an Employer</button>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container" style={{ paddingTop: 40, paddingBottom: 40 }}>
          <div className="footer-grid">
            <div style={{ maxWidth: 280 }}>
              ...
              <p style={{ color: "var(--muted)", fontSize: 13.5, marginTop: 14, lineHeight: 1.5 }}>Matching London's students with local placements. Starting in London, Ontario.</p>
            </div>
          ...
          <div style={{ borderTop: "1px solid var(--line)", marginTop: 32, paddingTop: 20, display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "space-between", color: "var(--muted-2)", fontSize: 13 }}>
            ...
            <span style={{ display: "flex", gap: 18 }}><a>Privacy</a><a>Terms</a><a>Accessibility</a></span>
          </div>
        </div>
      </footer>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        {links.map(l => <a key={l} style={{ color: "var(--muted)", fontSize: 13.5, cursor: "pointer" }}>{l}</a>)}
      </div>
    </div>
  );
}
